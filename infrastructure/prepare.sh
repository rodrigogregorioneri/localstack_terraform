...
cd ..
cd ./lambdas
for i in *.zip; do aws --endpoint-url=http://localhost:4566 lambda create-function --function-name "${i}" --zip-file "fileb://${i}" --handler index.handler --runtime nodejs12.x --role arn:aws:iam::000000000000:role/roleLambdaOlaMundo; done
